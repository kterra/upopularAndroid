...
//                    cv.put(UPAS_COLUMN_SINLONG, preCalculatedValues.get(UPAS_COLUMN_SINLONG));
    ...
    public void insertPHBrasilData(InputStream inStream, SQLiteDatabase db) {
...
//                    cv.put(UPAS_COLUMN_SINLONG, preCalculatedValues.get(UPAS_COLUMN_SINLONG));
    ...
    public Map<String, List<String>> getPHByQuery(String query, LatLng currentPos){
        ...
        Cursor res =  db.rawQuery( "select "+ VIRTUAL_TABLE_COLUMN_ID + ","+ Constants.NAME + ","+ Constants.STREET + "," + Constants.ZIPCODE + "," +
                ...
                " ORDER BY abs("+ Constants.LATITUDE +" - " +  currentPos.latitude  + ") + abs("+ Constants.LONGITUDE +" - "+ currentPos.longitude + ") LIMIT 40;", null );
        res.moveToFirst();
